	...
	if (Lampa.Storage.get('needRebootSettingExit')) {
  		var intervalSettings = setInterval(function() {
  			...
  			if (!elementSettings.length > 0){
				...
				showReload('За почистване на плъгина презаредете приложението!');
  			}
		}, 1000)
	}
}
...
if ($('DIV[data-name="' + itemName + '"]').find('.settings-param__status').hasClass('active')) {Lampa.Noty.show("Плъгина вече е инсталиран!")} else {
   ...
   if (!Lampa.Storage.get('needReboot')) {
	...
	// Делаем инъекцию скрипта для немедленной работы
		...
		setTimeout(function() {
			Lampa.Settings.update();
			Lampa.Noty.show("Плъгинът " + sourceName + " е успешно инсталиран")
		}, 300);
...
function deletePlugin(pluginToRemoveUrl) {
	...
	Lampa.Settings.update();
	Lampa.Noty.show("Плъгинът е успешно премахнат");
	Lampa.Storage.set('needRebootSettingExit', true);
	   settingsWatch();
};
...
/* Интерфейс */
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'TMDB Proxy',
						description: 'Прокси достъп до TMDB'
                            },
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Отзиви',
						description: 'Добавя в картата на филма бутони за отзиви'
					},
					onChange: function(value) {
						if (value == '1') {
							itemON('http://newtv.mail66.org/o.js', 'Отзиви', '@elenatv99', 'Feedback');
						}
						if (value == '2') {
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Приятни дреболии',
						description: 'Приставката ви позволява да избирате различни добавки от инсталатора (скрийнсейвъри, стил на бутони, стил на плейъра, часовник в плейъра и т.н.)'
					},
					onChange: function(value) {
						if (value == '1') {
							itemON('https://andreyurl54.github.io/diesel5/tricks.js', 'Приятни дреболии', '@AndreyURL54', 'Tricks');
						}
						if (value == '2') {
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Оценки от КиноПоиск и IMDB',
						description: 'Показване на оценките от KinoPoisk и IMDB в картата. Функционалността е подобна на частите от MODS, така че не трябва да се използват заедно'
					},
					onChange: function(value) {
						if (value == '1') {
							itemON('https://nb557.github.io/plugins/rating.js', 'Оценки от КиноПоиск и IMDB', '@t_anton', 'Rating');
						}
						if (value == '2') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Стар стил на меню(Отметки, харесвания, по-късно)',
						description: 'Стар стил на меню(Отметки, харесвания, по-късно)'
					},
					onChange: function(value) {
						if (value == '1') {
							itemON('http://github.freebie.tom.ru/want.js', 'Стар стил на меню(Отметки, харесвания, по-късно)', '@VitalikPVA', 'Want');
						}
						if (value == '2') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Нулиране на настройките на субтитрите',
						description: 'Нулиране на настройките на субтитрите по подразбиране'
					},
					onChange: function(value) {
						if (value == '1') {
						       itemON('https://nb557.github.io/plugins/reset_subs.js', 'Нулиране на настройките на субтитрите', '@t_anton', 'Sub_reset');
						}
						if (value == '2') {
	         ...
	         Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Анимационни филми',
						description: 'Плъгинът заменя менюто Аниме с Анимационни филми'
					},
					onChange: function(value) {
						if (value == '1') {
						       itemON('http://193.233.134.21/plugins/mult.js', 'Анимационни филми', '@AndreyURL54', 'Mult');
						}
						if (value == '2') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					onChange: function(value) {
						if (value == '1') {
						       itemON('https://cub.red/plugin/collections', 'Колекции', '@lampa', 'Collections');
						}
						if (value == '2') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Времето',
						description: 'Плъгинът ще показва последователно часа и времето, редувайки показанията'
					},
					onChange: function(value) {
						if (value == '1') {
						       itemON('https://bylampa.github.io/weather.js', 'Времето', '@scabrum', 'Weather');
						}
						if (value == '2') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Cub Off',
						description: 'Плъгинът премахва елементи, които предполагат регистрация за cub premium'
					},
					onChange: function(value) {
						if (value == '1') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Стилен интерфейс',
						description: 'Нов Стилен интерфейс за каталозите на TBDB и CUB. Ще се хареса на тези, които харесват интерфейса в Kinopoisk или netflix'
					},
					onChange: function(value) {
						if (value == '1') {
						       itemON('https://bylampa.github.io/interface.js', 'Стилен интерфейс', '@lampa', 'Style_interface_fix');
						}
						if (value == '2') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Допълнителни категории',
						description: 'Плъгинът ви позволява да добавяте категории към главното меню (документални филми, концерти и анимационни филми)'
					},
					onChange: function(value) {
						if (value == '1') {
						       itemON('https://lampame.github.io/main/nc/nc.js', 'Допълнителни категории', '@GwynnBleiidd', 'New_cat');
						}
						if (value == '2') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Допълнителни източници',
						description: 'Плъгиниът добавя Допълнителни източници на информация за филмите'
					},
					onChange: function(value) {
						if (value == '1') {
						       itemON('https://bylampa.github.io/source.js', 'Допълнителни източници', '@scabrum', 'New_source');
						}
						if (value == '2') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Start',
						description: 'Плъгинът ви позволява достъп до блокирани карти'
					},
					onChange: function(value) {
						if (value == '1') {
	          ...
	          Lampa.SettingsApi.addParam({
						...
						field: {
							name: 'Златна тема',
							description: 'Златна тема'
						},
						onChange: function(value) {
							if (value == '1') {
								itemON('https://bazzzilius.github.io/scripts/gold_theme.js', 'Златна тема', '@BazZziliuS', 'goldtheme');
							}
							if (value == '2') {
	        ...
	        Lampa.SettingsApi.addParam({
						...
						field: {
							name: 'Търсене на концерти',
							description: 'Плъгинът търси концерти чрез анализатора Jackett'
						},
						onChange: function(value) {
							if (value == '1') {
								itemON('https://lampame.github.io/main/cts.js', 'Търсене на концерти', '@GwynnBleiidd', 'concert_search');
							}
							if (value == '2') {
...
/* Управление */
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
                                    name: 'Изход',
                                    description: 'Плъгинът добавя позиция Изход в менюто на Lampa',
					},
					onChange: function(value) {
						if (value == '1') {
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Проверка за нова версии',
						description: 'Проверка за нова версия на приложения за Android TV'
					},
					onChange: function(value) {
						if (value == '1') {
							itemON('https://nemiroff.github.io/lampa/updater.js', 'Проверка за нова версии', '@nemiroff', 'New_Version');
							}
						if (value == '2') {
		...
		Lampa.SettingsApi.addParam({
					...
					//default: '1',
                            		...
                            		field: {
                                    		name: 'Горещи бутони',
                                    		description: 'Плъгинът извиква менюто на плейъра на лампата с помощта на бутоните на дистанционното управление: 5 - плейлист, 8 - аудио записи, 0 - субтитри, канал +/- следващ/предишен файл в плейлиста'
                            		},
					onChange: function(value) {
						if (value == '1') {
							itemON('https://nnmdd.github.io/lampa_hotkeys/hotkeys.js', 'Горещи бутони', '@nnmd', 'Hot_Buttons');
						}
						if (value == '2') {
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
                                    		...
                                    		description: 'Плъгинът работи на устройството Orsay, за Tizen трябва да актуализирате джаджата до версия 1.9.1'
					},
					onChange: function(value) {
						if (value == '1') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					field: {
                                    		name: 'Тип управление',
                                    		description: 'По време на инсталацията плъгинът ви позволява да изберете типа управление (Дистанционно управление без мишка, Дистанционно управление с мишка, Сензорен екран), както и да нулирате предварително избрания тип управление'
					},
					onChange: function(value) {
						if (value == '1') {
						       itemON('https://nemiroff.github.io/lampa/select_weapon.js', 'Тип управление', '@nemiroff', 'Select_Weapon');
						}
						if (value == '2') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					field: {
                                    		...
                                    		description: 'Приставката деактивира сензорното управление, ако е била активирана по погрешка'
					},
					onChange: function(value) {
						if (value == '1') {
						       itemON('https://nb557.github.io/plugins/not_mobile.js', 'Изключване на тъч управлени', '@t_anton', 'Touch_off');
						}
						if (value == '2') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					field: {
                                    		name: 'Wsoff',
                                    		description: 'Приставка за деактивиране на грешка (Request was denied for security) на по-стари версии на Android. Не инсталирайте, ако няма грешка'
					},
					onChange: function(value) {
						if (value == '1') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					field: {
                                    		name: 'Промяна на сървъра',
                                    		description: 'Плъгинът ви позволява да промените сървъра на приложението'
					},
					onChange: function(value) {
						if (value == '1') {
						       itemON('https://bylampa.github.io/redirect.js', 'Промяна на сървъра', '@scabrum', 'Redirect');
						}
						if (value == '2') {
...
/* Онлайн */
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Modss',
						description: 'Плъгинът ви позволява да гледате филми и телевизионни сериали онлайн. Има 17 балансъра, от които да избирате и различни добавки чрез менюто с настройки на Modss. За VIP 4K може да се свърже чрез телеграм бот @modssmy_bot'
					},
					onChange: function(value) {
						if (value == '1') {
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Online_Mod',
						description: 'Плъгинът ви позволява да гледате филми и телевизионни сериали онлайн. Налични са 7 балансъра, от които да избирате'
					},
					onChange: function(value) {
						if (value == '1') {
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
						...
						description: 'Аналог на онлайн плъгина от разработчика на приложението Lampa, но с нов информативен интерфейс за гледане на филми и сериали онлайн'
					},
					onChange: function(value) {
						if (value == '1') {
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
                                  		name: 'Онлайн BWA',
                                  		description: 'Плъгин за гледане на филми и сериали онлайн'
					},
                           		onChange: function(value) {
					if (value == '1') {
...
/* Торрент */
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Превключване на анализатори',
						description: 'Плъгинът ви позволява да превключвате между анализаторите на jackett от списък с вече попълнени правилни параметри. Елемент със списък с публични jackett-и ще се появи в настройките на анализатора'
					},
					onChange: function(value) {
						if (value == '1') {
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Tracks',
						description: 'Плъгинът замества името на аудио записи и субтитри в плейъра (работи само в торенти)'
					},
					onChange: function(value) {
						if (value == '1') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Настройка на торенти (Web OS, Tizen)',
						description: 'Добавка за телевизори, на които Lampa се инсталира през официалните LG Store и Tizen App Store. Разрешава показването на елементите Parser и Torrserver в настройките, които са необходими за гледане на торенти'
					},
					onChange: function(value) {
						if (value == '1') {
							itemON('http://cub.red/plugin/etor', 'Настройка на торенти', '@lampa', 'Setting_torrents');
						}
						if (value == '2') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Търсене локален TorrServer',
						description: 'Плъгинът ви позволява да търсите за вашия локален TorrServer'
					},
					onChange: function(value) {
						if (value == '1') {
							itemON('http://193.233.134.21/plugins/checker.js', 'Търсене локален TorrServer', '@AndreyURL54', 'Check_server');
						}
						if (value == '2') {
	         ...
	         Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Добавяне на торенти',
						description: 'Плъгинът добавя връзка към торент клиенти като qBittorent, Transmission с последваща възможност за изтегляне на торенти чрез тях за локално гледане'
					},
					onChange: function(value) {
						if (value == '1') {
							itemON('https://lampame.github.io/td/td.js', 'Добавяне на торенти', '@feliks', 'Torr_download');
						}
						if (value == '2') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Втори бутон торенти',
						description: 'Плъгинът добавя втори бутон за торенти'
					},
					onChange: function(value) {
						if (value == '1') {
							itemON('https://github.freebie.tom.ru/torrents.js', 'Втори бутон торенти', '@VitalikPVA', 'second_but_torrents');
						}
						if (value == '2') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					field: {
						...
						description: 'Плъгинът автоматично предоставя свободен torrserver от лична база данни'
					},
					onChange: function(value) {
						if (value == '1') {
...
/* Телеканалы */
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Дизел ТВ',
						description: 'Плъгин за безплатно гледане на ТВ канали и рекламни плейлисти с ТВ програма и архивен запис'
					},
					onChange: function(value) {
						if (value == '1') {
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Kulik',
						description: 'Плъгин за гледане на IPTV канали, сортирани в различни категории. Възможно е да промените стила на плъгина, сървъра за излъчване, както и да добавите канали към любими'
					},
					onChange: function(value) {
						if (value == '1') {
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
							name: 'IPTV',
							description: 'Плъгин за гледане на IPTV канали. Сортиране на канали в групи и възможност за добавяне на канали към любими. Работи само с вашия собствен плейлист, добавен на уебсайта https://cub.watch/iptv'
					},
					onChange: function(value) {
						if (value == '1') {
	        ...
	        Lampa.SettingsApi.addParam({
					...
					field: {
							name: 'Hack TV',
							description: 'Плъгин за гледане на IPTV канали'
					},
					onChange: function(value) {
						if (value == '1') {
...
/* Радио */
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Радио Record',
						description: 'Слушане на радио от радиостанция Radio Record. Налични са повече от 50 жанра, има поне един по ваш вкус'
					},
					onChange: function(value) {
						if (value == '1') {
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
						...
						description: 'Все същият плъгин за Radio Record, но с един списък от станции без разделение по жанрове'
					},
					onChange: function(value) {
                        			if (value == '1') {
...
/* Клубника */
		...
		Lampa.SettingsApi.addParam({
					...
					field: {
						name: 'Ягодка',
						description: 'Плъгин за гледане на 18+ съдържание'
					},
					onChange: function(value) {
						if (value == '1') {
							itemON('https://bwa.to/s', 'Ягодка', '@rik', 'Sisi');
						}
						if (value == '2') {
